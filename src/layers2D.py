...
def conv2d_block(
    ...
    output = hparams.HID_ACT_FUNC(conv)
...
def transpose_block(
    ...
    conv = Conv2DTranspose(
        ...
        kernel_size=kernel_size,
        padding="same",
        strides=(shape_xskip[1] // shape_x[1], shape_xskip[2] // shape_x[2]),
        kernel_initializer="he_normal",
    )(input_tensor)
    conv = hparams.HID_ACT_FUNC(conv)
...
def attn_gate_block(x, g, inter_shape):
    ...
    output = Conv2D(filters=shape_x[3], kernel_size=1, strides=1, padding="same")(
        attn_coefficients
    )
...
def gate_signal(input_tensor, batchnorm=True):
